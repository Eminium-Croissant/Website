...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!loginRes.ok || !loginData.token) {
      ...
      return res.status(loginRes.status || 401).send(process.env.NODE_ENV === 'development' ? `[DEV] Google OAuth failed: ${errorMsg}. Backend: ${apiBase}` : 'Authentication failed. Please try again.');
    }
    ...
    res.status(500).send(isDev ? `[DEV] Google OAuth error: ${msg}. Check API_BASE_URL or ensure backend is running.` : 'An error occurred during authentication.');
  }
}
