...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const message = isConnRefused ? `Connection refused when reaching API at ${url}. Verify API_BASE_URL and ensure the backend is reachable from this runtime.` : `Fetch to upstream API failed: ${err && err.message ? err.message : String(err)}`;
