              ...
              return (
                <>
                  {parts.map((part, idx) => {
                    if (part === 'settings') {
                      return (
                        <Link href="/settings" key={idx} className="text-neon-blue hover:text-neon-purple transition-colors">
                          settings
                        </Link>
                      )
                    }
                    if (part === 'documentation') {
                      return (
                        <Link href="/api-docs" key={idx} className="text-neon-blue hover:text-neon-purple transition-colors">
                          documentation
                        </Link>
                      )
                    }
    ...
    },
    {
      ...
      content: (
        <ListSection
          ...
          items={[t('index.overview.lobby.1'), t('index.overview.lobby.2'), t('index.overview.lobby.3'), t('index.overview.lobby.4')]}
        />
      )
    },
    {
    ...
    const demoGames: Game[] = [
      {
        ...
        description:
          "Explorez l'univers infini dans cette aventure spatiale épique. Combattez des ennemis, découvrez de nouveaux mondes et construisez votre empire galactique.",
        ...
        <div className="flex justify-center mt-6 gap-2">
          {games.map((_, index) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-neon-blue' : 'bg-glass-border hover:bg-glass-accent'}`}
            />
          ))}
        </div>
              ...
              <p className="text-lg mb-6" style={{ color: 'var(--glass-text-secondary)' }}>
                <span
                  dangerouslySetInnerHTML={{
                    __html: t('index.openSource.description.desktop')
                  }}
                />
              </p>
            </div>
          ...
          <div className="glass-content-card !mt-4 !mx-0 mb-6">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-sm mb-4" style={{ color: 'var(--glass-text-secondary)' }}>
                <span
                  dangerouslySetInnerHTML={{
                    __html: t('index.openSource.description.mobile')
                  }}
                />
              </p>
            </div>
