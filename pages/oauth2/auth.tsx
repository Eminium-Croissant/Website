  ...
  return (
    <div className='glass-page-container flex justify-center items-center min-h-screen'>
      <div className="glass-content-card w-full mx-auto mt-0 rounded-[18px] shadow-xl p-7 text-white font-['Segoe_UI',Arial,sans-serif] flex flex-col items-center justify-start relative">
        ...
        <div className='w-full flex-1 flex flex-col min-h-[80px] mb-8'>{(error || missingParams) && <div className='text-white bg-red-700 rounded-lg px-[14px] py-[10px] text-[1.01rem] text-center w-full'>{missingParams ? t('oauth2.auth.missingParams') : error}</div>}</div>
        ...
        <div className='w-full flex flex-col items-center gap-3 mb-6'>
          ...
          {!authLoading && user && !missingParams && (
            <button onClick={handleAuth} disabled={loading} className={`w-full glass-button-neon ${loading ? 'opacity-60 cursor-not-allowed' : ''}`}>
