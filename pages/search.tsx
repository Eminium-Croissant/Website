...
const SearchPage: React.FC = () => {
  ...
  const query = typeof router.query.q === 'string' ? router.query.q : Array.isArray(router.query.q) ? router.query.q[0] || '' : ''
            ...
            {items.map((item) => (
              ...
              >
                <div className="glass-card p-4 hover:scale-105 transition-transform duration-300 hover:shadow-glass-glow">
                  <div className="flex items-center gap-4">
                    <CachedImage
                      src={`/items-icons/${item.iconHash || item.itemId || 'default.avif'}`}
                      alt={item.name}
                      className="w-16 h-16 object-contain rounded-xl glass-card border-2 border-glass-border"
                    />
                    <div className="flex-1">
