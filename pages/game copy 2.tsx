...
const Library: React.FC<{ gameFromQuery?: Game }> = ({ gameFromQuery }) => {
  const { user, token } = useAuth();
  const router = useRouter();
  const { t } = useTranslation("common");

  const [games, setGames] = useState<Game[]>([]);
  const [selected, setSelected] = useState<Game | null>(gameFromQuery || null);
  ...
  useEffect(() => {
    if (!token) return;
    const gameIdFromQuery = router.query.gameId as string | undefined;
    fetch("/api/games/" + gameIdFromQuery, {
      method: "GET",
      headers: {
  ...
  const handleSelect = (game: Game) => {
    ...
    // localStorage.setItem("lastSelectedGameId", game.gameId); // <-- à supprimer
  };
  ...
  const handleTransferGame = async () => {
    ...
    try {
      const response = await fetch(`/api/games/${query.gameId}/transfer`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const confirmBuy = async () => {
    ...
    const query = router.query;
    try {
      const res = await fetch(`/api/games/${query.gameId}/buy`, {
        method: "POST",
  ...
  // Si gameFromQuery existe, on l'affiche en priorité
  const gameToDisplay = gameFromQuery;
          ...
          {!gameToDisplay ? (
            ...
            <div className="glass-content-card h-full" style={{ overflowY: "scroll" }}>
              ...
              <div className="relative h-64 mb-6 rounded-xl overflow-hidden">
                ...
                <img src={`/banners-icons/${gameToDisplay.bannerHash}`} alt={gameToDisplay.name} className="w-full h-full object-cover" loading="lazy" />
                <div className="absolute inset-0 bg-gradient-to-t from-dark-primary via-transparent to-transparent"></div>
                <div className="absolute bottom-4 left-4 flex items-center gap-4">
                  <img src={`/games-icons/${gameToDisplay.iconHash}`} alt={gameToDisplay.name} className="w-20 h-20 rounded-xl object-cover glass-card border-2 border-glass-border" loading="lazy" />
                  <div>
                ...
                <div className="absolute bottom-4 right-4 flex gap-4">
                  ...
                  <button
                    ...
                    disabled={isGifting || !gameToDisplay}
                    type="button"
                  >
                    {t("shop.giftWithPrice", { price: gameToDisplay?.price ?? 0 })}
                  </button>
                </div>
              </div>
              ...
              {gameToDisplay.description && (
                <div className="mt-6">
                  <MarkdownDescription>{gameToDisplay.description}</MarkdownDescription>
                </div>
              )}
            </div>
          )}
    ...
    try {
      ...
      if (res.ok) {
        ...
        ogMeta = {
          ...
          bannerUrl: game.bannerHash ? `https://croissant.gg/banners-icons/${game.bannerHash}` : "https://croissant.gg/assets/launcher.png",
...
const ExportedComponent = (props) => {
  ...
  if (props.ogMeta) {
    return (
      <>
        <OgGameMetaLinks {...props.ogMeta} />
        {user ? <Library {...props} /> : <Login />}
      </>
    );
  }
