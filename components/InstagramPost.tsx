...
export default function InstagramPost({ title = 'Croissant', subtitle = 'Gaming Platform', description = "Un système d'inventaire créatif et réutilisable, open source, évolutif, API à gogo et technologie réseau au top", logo = '🥐', features = ['Gaming', 'Trading', 'API', 'Open Source'], theme = 'default', size = 'medium' }: InstagramPostProps) {
  const themeConfig = {
    default: {
        ...
        <div className={`relative z-10 h-full flex flex-col justify-between ${currentSize.padding}`}>
          <div className={`flex items-center justify-between ${currentSize.gap}`}>
            <div className='flex items-center gap-3'>
              <div className={`${currentSize.logoSize} flex items-center justify-center w-16 h-16 rounded-2xl glass-card glass-glow`}>{logo}</div>
              <div>
                <h1 className={`${currentSize.titleSize} font-bold text-glass-text`}>{title}</h1>
                <p className={`${currentSize.subtitleSize} text-glass-text-secondary`}>{subtitle}</p>
              </div>
            </div>
            <FontAwesomeIcon icon={currentTheme.icon} className='text-2xl opacity-30' style={{ color: currentTheme.accentColor }} />
          </div>

          <div className='flex-1 flex items-center justify-center text-center'>
            <p className={`${currentSize.descriptionSize} text-glass-text leading-relaxed max-w-[80%]`}>{description}</p>
          </div>
          ...
          <div className='flex justify-center items-center gap-2 mt-4'>
            {[...Array(5)].map((_, i) => (
              <FontAwesomeIcon key={i} icon={faStar} className={`${size === 'small' ? 'text-xs' : 'text-sm'} opacity-60`} style={{ color: currentTheme.accentColor }} />
            ))}
          </div>
        </div>
