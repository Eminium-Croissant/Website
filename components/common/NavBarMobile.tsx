...
export default function NavBarMobile() {
  ...
  const UserBlock = ({ loading, user }: any) => (
    <div className="flex items-center gap-3 mb-4">
      ...
      <Link href="/profile">
        <CachedImage
          src={loading ? '/avatar/default.avif' : `/avatar/${user.role || user.id}`}
          alt="avatar"
    ...
    <header className="w-full bg-[#23242a] text-[#e2e8f0] border-b border-secondary py-0.5 shadow-sm relative z-10">
      ...
      <nav>
        {drawerOpen && (
          <>
            <div
              className={`fixed top-0 left-0 w-[85vw] max-w-[380px] h-screen bg-[#23242a] z-[9999] shadow-lg flex flex-col transition-transform duration-250 ease-[cubic-bezier(.4,0,.2,1)] ${drawerVisible ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'}`}
            >
