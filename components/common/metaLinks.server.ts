...
export async function getMetaLinksProps(locale = 'en') {
  ...
  const metaLinksTitle =
    common?.index?.hero?.title ||
    common?.index?.title ||
    common?.launcher?.title ||
    common?.apiDocs?.title ||
    'Croissant Inventory System';

  const metaDescription =
    common?.index?.hero?.subtitle ||
    common?.index?.description ||
    common?.apiDocs?.intro ||
    common?.index?.topspan ||
