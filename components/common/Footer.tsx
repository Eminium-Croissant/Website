...
export default function Footer() {
  ...
  return (
    <footer
      className={`w-full glass-bg-gradient text-glass-text-secondary text-[0.92rem] text-center py-6 px-0 border-t border-glass-border backdrop-blur-md ${
    ...
    >
      <div className="max-w-[1400px] mx-auto px-6">
        <div className="flex items-center justify-center gap-4 flex-wrap">
          <span className="bg-gradient-to-r from-neon-blue to-neon-purple bg-clip-text text-transparent font-semibold">
            {t("footer.copyright")}
          </span>
          {footerLinks.map((link, idx) => (
            <React.Fragment key={link.href}>
              ...
              <Link
                href={link.href}
                className="text-neon-blue no-underline mx-2 hover:text-neon-purple transition-all duration-300 hover:scale-105"
              >
                {link.label}
              </Link>
            </React.Fragment>
