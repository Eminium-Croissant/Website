  ...
  return (
    <div className={`relative ${className}`}>
      <button 
        ...
        className="flex items-center gap-2 px-3 py-2 rounded-lg bg-glass-accent border border-glass-border text-glass-text hover:bg-glass-secondary transition-all duration-200"
      >
          ...
          <div className="absolute top-full right-0 mt-1 w-48 z-50 bg-glass-primary border border-glass-border rounded-lg shadow-glass max-h-64 overflow-y-auto">
            {languages.map((language) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-2 text-left hover:bg-glass-accent transition-colors duration-200 ${
                  language.code === locale ? 'bg-glass-accent border-l-2 border-l-neon-blue' : ''
                }`}
              >
