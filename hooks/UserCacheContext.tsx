...
export const UserCacheProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getUser = useCallback(
    async (userId: string, cache = true, admin = false): Promise<UserCacheUser | null> => {
  ...
  return <UserCacheContext.Provider value={{ getUser, cacheUser, cache: userCacheRef.current }}>{children}</UserCacheContext.Provider>;
};
